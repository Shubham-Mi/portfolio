...
Single-page Next.js 15 App Router portfolio. All visible content lives on one route (`/`), with hash-based in-page navigation (`#about`, `#skills`, etc.).
...
- `next.config.ts` — Rewrites that proxy `/project/<slug>` to external Vercel-deployed apps.
...
**Adding a proxied project:** Add two rewrite rules to `next.config.ts` — one for the root path and one for `/:path*` — pointing to the deployed Vercel URL.
